Agio pool
====================
Formerly known as cryptonote-forknote-pool, forked from TurtleCoin and adjusted for Agio coin.
...
Rebuild the coin from the following GitHub repository: https://github.com/agioprojects/agio using the instructions in the repository Readme.md file.

-  Start the daemon in the RPC mode

```
./agiod --rpc-bind-port=11213
```

-  Generate the pool wallet using walletd

```
./walletd --container-file=YOURWALLETNAME --container-password=YOURWALLETPASSWORD --daemon-port=11213 --bind-port=8082 --rpc-legacy-security --generate-container
```

-  Start the wallet using walletd

```
./walletd --container-file=YOURWALLETNAME --container-password=YOURWALLETPASSWORD --daemon-port=11213 --bind-port=8082 --rpc-legacy-security
```
...
```
git clone https://github.com/agioprojects/agio-pool.git pool
```
